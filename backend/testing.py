...
if __name__ == "__main__":
    ...
    assert isinstance(
        matched_format, dict
    ), "matched_format returned is not a dictionary"
    ...
    for score in matched_format["Score"]:
        assert (
            score > 0.60
        ), "One of the scores matched is less than 0.60, invalid match"
    ...
    for mentor_id in match.mentor_assigned_count.keys():
        assert (
            len(match.mentor_assigned_count[mentor_id]) <= JUNIOR_MAX
        ), f"One of the mentors has more than JUNIOR_MAX={JUNIOR_MAX}"
