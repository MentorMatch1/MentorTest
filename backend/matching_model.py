...
class Score_Calculator:
    ...
    def embed_interests(self, interests):
        for i in range(len(interests)):
            interests[i] = ollama.embeddings(
                model="mxbai-embed-large", prompt=interests[i]
            )["embedding"]
        ...
        for mentor_id in self.mentor_id_list:
            for mentee_id in self.mentee_id_list:
                ...
                self.scores_df.loc[mentor_id, mentee_id] = round(
                    (self.compatibility_scores[mentor_program][mentee_program] * 0.35)
                    + self.scores_df.loc[mentor_id, mentee_id],
                    5,
                )
    ...
    def compare_residence(self):
        for mentor_id in self.mentor_id_list:
            for mentee_id in self.mentee_id_list:
                mentor_column = self.mentor_df.loc[mentor_id, "Mentor Residence"]
                mentee_column = self.mentee_df.loc[mentee_id, "Mentee Residence"]
...
class Matching:
    ...
    def __init__(
        self, mentor_df, mentee_df, scores_df, matched_format, mentor_vars, mentee_vars
    ):
        self.mentor_df = mentor_df.copy()
        self.mentee_df = mentee_df.copy()
    ...
    def assign_mentor_mentee(self, mentee_id, index, largest_match_score):
        ...
        for i in range(len(self.matched_format["Mentor Residence"])):
            if not isinstance(self.matched_format["Mentor Residence"][i], bool):
                self.matched_format["Mentor Residence"][i] = bool(
                    self.matched_format["Mentor Residence"][i]
                )

        for i in range(len(self.matched_format["Mentee Residence"])):
            if not isinstance(self.matched_format["Mentee Residence"][i], bool):
                self.matched_format["Mentee Residence"][i] = bool(
                    self.matched_format["Mentee Residence"][i]
                )

    def assignment(self) -> dict:
        for percentage in range(90, 50, -10):
            logger.info(f"iteration ------------ {percentage}")
            for mentee_id in list(self.mentee_id_list):
                ...
                if best_mentor_index > -1:
                    self.assign_mentor_mentee(
                        mentee_id, best_mentor_index, largest_match_score
                    )

                    logger.info(
                        f"Final Match - Mentee ID: {mentee_id} with Mentor ID: {self.mentor_id_list[best_mentor_index]} score: {largest_match_score}"
                    )
                    logger.info("------------------------------------------------")
    ...
    def mentor_matches(self) -> dict:
        ...
        for key in self.mentor_assigned_count.keys():
            ...
            for i in range(1, len(mm_data_keys) - 2):
                self.mentor_assigned_data[mm_data_keys[i]].append(
                    self.mentor_df.loc[key][mm_data_keys[i]]
                )

            self.mentor_assigned_data["Mentor Assigned Count"].append(
                len(self.mentor_assigned_count[key])
            )
            self.mentor_assigned_data["Mentees Assigned"].append(
                "".join(str(mentee) + " " for mentee in self.mentor_assigned_count[key])
            )
